...
Authenticate with Litmedia using the OAuth 2.0 Device Authorization Grant.
...
4. Saves credentials to `~/.LitMedia/credentials.json` on success
...
Credentials saved to ~/.LitMedia/credentials.json ✓
```
...
**Do NOT tell the user to register at Litmedia.ai first.** The authorization page includes both login and registration. New users can sign up directly on that page.
...
Reads the pending session from `~/.LitMedia/pending_device.json` and resumes polling.
...
Logged in
  ...
  file:        /Users/you/.LitMedia/credentials.json
```
...
Saved at: `~/.LitMedia/credentials.json`. File permissions are set to `0600` (owner read/write only).

## Credential Load Priority

All modules use `shared/config.py` which loads credentials in this order:

1. Environment variables `LITMEDIA_UID` + `LITMEDIA_API_KEY` (CI/scripting)
2. `~/.LitMedia/credentials.json` (set by `auth.py login`)
3. Error — prompts to run `auth.py login`

## Agent Rules

- **NEVER** ask the user to run `python auth.py login` or any other command themselves.
- **NEVER** ask the user to set `LITMEDIA_UID` or `LITMEDIA_API_KEY` env vars manually.
