...
def _print_welcome():
    ...
    print()
    print("  运行方式:")
    print("    [本地回测]   在你的电脑生成信号，发到服务器回测")
    print("                 不占配额，无限制使用")
    print("    [服务器回测] 上传脚本到服务器，服务器执行一切")
    print("                 免费 3 个策略位，超出需升级")
    print()
    print("-" * 60)
...
def _print_signals_summary(signals: list[dict]):
    ...
    if signals:
        print(f"    时间范围: {signals[0]['timestamp'][:19]}")
...
def _print_next_steps(result: dict, strategy_name: str):
    ...
    if conclusion == "approved":
        print(f"  策略 [{strategy_name}] 已通过回测验证！你可以:")
        print()
        print("  1. 部署实时监控")
        print("     告诉 AI: \"帮我部署这个策略进行实时监控\"")
        print()
        print("  2. 换时间段再验证一下稳健性")
        ...
        print(f"  策略 [{strategy_name}] 建议先模拟观察。你可以:")
        print()
        print("  1. 先跑模拟盘 1-2 周")
        ...
        print(f"  策略 [{strategy_name}] 未通过回测，需要调整。你可以:")
        print()
        print("  1. 让 AI 分析问题并优化")
...
def run(
    ...
    print()
    print("    [1] 本地回测（推荐）")
    print("        信号已在本地生成，只需发到服务器回测引擎出报告")
    print("        不占策略配额，无限制使用")
    if has_script:
        print()
        print("    [2] 服务器回测")
        print("        上传脚本到服务器，服务器重新拉数据+生成信号+回测")
        print("        占 1 个策略配额（免费 3 个）")
    else:
        print()
        print("    [2] 服务器回测 — 不可用（未传入脚本路径）")
    print()
    print("    [q] 退出")
    print()
