---
name: lobster-says
description: "🦞 虾说——你的专属共情虾。每天早晚给你一句关心的话，让你觉得被看到了。"
user-invocable: true
...
- `success=true` 且 `cron_registered=false`：明确告诉用户**虾已经创建成功**，只是定时推送注册暂时失败，稍后补跑 `setup-cron.sh` 即可；**不要**把它描述成“后端整体不可用”或“初始化失败”
