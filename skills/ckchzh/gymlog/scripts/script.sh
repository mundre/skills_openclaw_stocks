...
DATA_DIR="${HOME}/.local/share/gymlog"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "gymlog v2.0.0"; }

_help() {
    echo "Gymlog v2.0.0 — sysops toolkit"
    echo ""
    echo "Usage: gymlog <command> [args]"
    echo ""
    echo "Commands:"
    echo "  scan               Scan"
...
_search() {
    local term="${1:?Usage: gymlog search <term>}"
...
case "${1:-help}" in
    scan)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/scan.log" 2>/dev/null || echo "  No entries yet. Use: gymlog scan <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/scan.log"
            local total=$(wc -l < "$DATA_DIR/scan.log")
            echo "  [Gymlog] scan: $input"
    ...
    monitor)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/monitor.log" 2>/dev/null || echo "  No entries yet. Use: gymlog monitor <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/monitor.log"
            local total=$(wc -l < "$DATA_DIR/monitor.log")
            echo "  [Gymlog] monitor: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: gymlog report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Gymlog] report: $input"
    ...
    alert)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/alert.log" 2>/dev/null || echo "  No entries yet. Use: gymlog alert <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/alert.log"
            local total=$(wc -l < "$DATA_DIR/alert.log")
            echo "  [Gymlog] alert: $input"
    ...
    top)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/top.log" 2>/dev/null || echo "  No entries yet. Use: gymlog top <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/top.log"
            local total=$(wc -l < "$DATA_DIR/top.log")
            echo "  [Gymlog] top: $input"
    ...
    usage)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/usage.log" 2>/dev/null || echo "  No entries yet. Use: gymlog usage <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/usage.log"
            local total=$(wc -l < "$DATA_DIR/usage.log")
            echo "  [Gymlog] usage: $input"
    ...
    check)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/check.log" 2>/dev/null || echo "  No entries yet. Use: gymlog check <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/check.log"
            local total=$(wc -l < "$DATA_DIR/check.log")
            echo "  [Gymlog] check: $input"
    ...
    fix)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/fix.log" 2>/dev/null || echo "  No entries yet. Use: gymlog fix <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/fix.log"
            local total=$(wc -l < "$DATA_DIR/fix.log")
            echo "  [Gymlog] fix: $input"
    ...
    cleanup)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/cleanup.log" 2>/dev/null || echo "  No entries yet. Use: gymlog cleanup <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/cleanup.log"
            local total=$(wc -l < "$DATA_DIR/cleanup.log")
            echo "  [Gymlog] cleanup: $input"
    ...
    backup)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/backup.log" 2>/dev/null || echo "  No entries yet. Use: gymlog backup <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/backup.log"
            local total=$(wc -l < "$DATA_DIR/backup.log")
            echo "  [Gymlog] backup: $input"
    ...
    restore)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/restore.log" 2>/dev/null || echo "  No entries yet. Use: gymlog restore <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/restore.log"
            local total=$(wc -l < "$DATA_DIR/restore.log")
            echo "  [Gymlog] restore: $input"
    ...
    log)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/log.log" 2>/dev/null || echo "  No entries yet. Use: gymlog log <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/log.log"
            local total=$(wc -l < "$DATA_DIR/log.log")
            echo "  [Gymlog] log: $input"
    ...
    benchmark)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/benchmark.log" 2>/dev/null || echo "  No entries yet. Use: gymlog benchmark <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/benchmark.log"
            local total=$(wc -l < "$DATA_DIR/benchmark.log")
            echo "  [Gymlog] benchmark: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: gymlog compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [Gymlog] compare: $input"
    ...
    *)
        echo "Unknown: $1 — run 'gymlog help'"
        exit 1
        ;;
esac