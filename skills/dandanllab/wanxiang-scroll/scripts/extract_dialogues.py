...
OUTPUT_DIR = r"d:\projects\wanxiang-scroll\references\chapter-12-book-analysis\dialogues"
...
def analyze_dialogue_style(dialogues):
    ...
    if all_dialogues:
        style_info["avg_length"] = sum(len(d) for d in all_dialogues) // len(all_dialogues)
...
def main():
    ...
    avg_length = sum(s["avg_length"] for s in all_styles.values()) // len(all_styles) if all_styles else 0
