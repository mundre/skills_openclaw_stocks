...
⚠️ 学习用途声明：本脚本下载的所有小说文件仅供学习参考使用，不得用于任何商业用途或其他非学习目的。
...
class FqNovelCrawler:
    ...
    def _test_connection(self):
        ...
        try:
            ...
            if isinstance(result, dict) and result.get('code') == 200:
                ...
                print(f"⚠️ API 连接异常: code = {result.get('code', 'N/A')}")
    ...
    def search(self, keyword: str, limit: int = 20) -> List[Dict]:
        ...
        search_url = f"{self.base_url}/search?key={keyword}&tab_type=3"
        ...
        for i, book in enumerate(books, 1):
            book_data_list = book.get("book_data", [])
            if not book_data_list:
                continue
            
            book_info = book_data_list[0]
            
            book_id = book_info.get("book_id", "")
            title = book_info.get("book_name", "未知")
            author = book_info.get("author", "未知")
            category = book_info.get("category", "未知")
            word_count = int(book_info.get("word_number", 0))
            chapter_count = int(book_info.get("serial_count", 0))
            score = book_info.get("score", "未知")
            tomato_status = book_info.get("tomato_book_status", "")
            status = "完结" if tomato_status == "1" else "连载中"
            ...
            def normalize_title(title: str) -> str:
                ...
                def cn_to_int(cn_str: str) -> int:
                    special_nums = {
                        ...
                        '二十':20,'三十':30,'四十':40,'五十':50,'六十':60,'七十':70,'八十':80,'九十':90,'一百':100
                    }
                    ...
                    if len(cn_str) == 3 and cn_str[0] in cn_nums and cn_str[1] == '十' and cn_str[2] in cn_nums:
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="⚠️ 学习用途声明：本脚本下载的所有小说文件仅供学习参考使用"
    )
