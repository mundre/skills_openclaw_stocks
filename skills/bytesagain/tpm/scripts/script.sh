#!/usr/bin/env bash
# tpm -- Total productive maintenance tracker
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${TPM_DIR:-$HOME/.tpm}"
...
show_help() {
    cat << EOF
tpm v$VERSION -- Total productive maintenance tracker
...
cmd_add() {
    local value="${1:?Usage: tpm add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: tpm search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: tpm remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "tpm v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: tpm help"; exit 1 ;;
esac
