#!/usr/bin/env bash
# maker — tags: [maker, tool, utility]
# Powered by BytesAgain | bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MAKER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/maker}"
...
cmd_init() {
    ensure_dirs
    echo "[maker] Initialized at $DATA_DIR"
    echo "  Entries file: $ENTRIES"
    echo "  Config file:  $CONFIG"
    echo ""
    echo "Ready. Try: maker add \"your first item\""
}

cmd_add() {
    ensure_dirs
    local text="${*:?Usage: maker add <text>}"
    local ts=$(now_ts)
    local id=$(($(entry_count) + 1))
    printf '{"id":%d,"text":"%s","created":"%s"}\n' "$id" "$text" "$ts" >> "$ENTRIES"
    echo "[maker] #$id added: $text"
}
...
cmd_list() {
    ...
    if [ "$total" -eq 0 ]; then
        echo "[maker] No entries yet. Use: maker add <text>"
        return
    fi
    echo "[maker] $total entries:"
    echo ""
    local n=0
...
cmd_show() {
    ensure_dirs
    local id="${1:?Usage: maker show <id>}"
    local total=$(entry_count)
...
cmd_remove() {
    ensure_dirs
    local id="${1:?Usage: maker remove <id>}"
    local total=$(entry_count)
    if [ "$id" -lt 1 ] || [ "$id" -gt "$total" ]; then
        echo "Error: id must be 1-$total"; return 1
    fi
    ...
    echo "[maker] Removed #$id: $text"
}

cmd_search() {
    ensure_dirs
    local term="${1:?Usage: maker search <term>}"
    local found=0
    local n=0
    while IFS= read -r line; do
        n=$((n + 1))
    ...
    echo "[maker] Found $found matches for '$term'"
}
...
case "${1:-help}" in
    ...
    version|-v)     echo "maker v$VERSION" ;;
