#!/usr/bin/env bash
# workflow-builder - Design reference and helper tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${WORKFLOW_BUILDER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/workflow-builder}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
workflow-builder v$VERSION

Design reference and helper tool

Usage: workflow-builder <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "workflow-builder v$VERSION" ;;
