#!/usr/bin/env bash
# debate-helper - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${DEBATE_HELPER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/debate-helper}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
debate-helper v$VERSION

Multi-purpose utility tool

Usage: debate-helper <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "debate-helper v$VERSION" ;;
