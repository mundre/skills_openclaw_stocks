#!/usr/bin/env bash
# treynor — Treynor reference tool. Use when working with treynor in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
treynor v$VERSION — Treynor Reference Tool

Usage: treynor <command>
...
## What is Treynor?
Treynor (treynor) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with treynor.

## Key Concepts
- Core treynor principles and fundamentals
- How treynor fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Treynor Matters
Understanding treynor is critical for:
...
## Getting Started
1. Understand the basic treynor concepts
...
cmd_formulas() {
    cat << 'EOF'
# Treynor — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Treynor — Instruments & Tools Overview

## Primary Instruments
- Core tools used in treynor operations
...
cmd_glossary() {
    cat << 'EOF'
# Treynor — Key Terms & Definitions

## Core Terminology
- **Treynor**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "treynor v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: treynor help"; exit 1 ;;
esac
