#!/usr/bin/env bash
# Currconv — finance tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

DATA_DIR="${HOME}/.local/share/currconv"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }

_version() { echo "currconv v2.0.0"; }

_help() {
    echo "Currconv v2.0.0 — finance toolkit"
    echo ""
    echo "Usage: currconv <command> [args]"
    echo ""
    echo "Commands:"
    echo "  record             Record"
...
_search() {
    local term="${1:?Usage: currconv search <term>}"
...
case "${1:-help}" in
    record)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/record.log" 2>/dev/null || echo "  No entries yet. Use: currconv record <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/record.log"
            local total=$(wc -l < "$DATA_DIR/record.log")
            echo "  [Currconv] record: $input"
    ...
    categorize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/categorize.log" 2>/dev/null || echo "  No entries yet. Use: currconv categorize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/categorize.log"
            local total=$(wc -l < "$DATA_DIR/categorize.log")
            echo "  [Currconv] categorize: $input"
    ...
    balance)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/balance.log" 2>/dev/null || echo "  No entries yet. Use: currconv balance <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/balance.log"
            local total=$(wc -l < "$DATA_DIR/balance.log")
            echo "  [Currconv] balance: $input"
    ...
    trend)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/trend.log" 2>/dev/null || echo "  No entries yet. Use: currconv trend <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/trend.log"
            local total=$(wc -l < "$DATA_DIR/trend.log")
            echo "  [Currconv] trend: $input"
    ...
    forecast)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/forecast.log" 2>/dev/null || echo "  No entries yet. Use: currconv forecast <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/forecast.log"
            local total=$(wc -l < "$DATA_DIR/forecast.log")
            echo "  [Currconv] forecast: $input"
    ...
    export-report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/export-report.log" 2>/dev/null || echo "  No entries yet. Use: currconv export-report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/export-report.log"
            local total=$(wc -l < "$DATA_DIR/export-report.log")
            echo "  [Currconv] export-report: $input"
    ...
    budget-check)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/budget-check.log" 2>/dev/null || echo "  No entries yet. Use: currconv budget-check <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/budget-check.log"
            local total=$(wc -l < "$DATA_DIR/budget-check.log")
            echo "  [Currconv] budget-check: $input"
    ...
    summary)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/summary.log" 2>/dev/null || echo "  No entries yet. Use: currconv summary <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/summary.log"
            local total=$(wc -l < "$DATA_DIR/summary.log")
            echo "  [Currconv] summary: $input"
    ...
    alert)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/alert.log" 2>/dev/null || echo "  No entries yet. Use: currconv alert <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/alert.log"
            local total=$(wc -l < "$DATA_DIR/alert.log")
            echo "  [Currconv] alert: $input"
    ...
    history)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/history.log" 2>/dev/null || echo "  No entries yet. Use: currconv history <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/history.log"
            local total=$(wc -l < "$DATA_DIR/history.log")
            echo "  [Currconv] history: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: currconv compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [Currconv] compare: $input"
    ...
    tax-note)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/tax-note.log" 2>/dev/null || echo "  No entries yet. Use: currconv tax-note <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/tax-note.log"
            local total=$(wc -l < "$DATA_DIR/tax-note.log")
            echo "  [Currconv] tax-note: $input"
    ...
    *)
        ...
        echo "Run 'currconv help' for available commands."
        exit 1
        ;;
esac