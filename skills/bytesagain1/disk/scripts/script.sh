...
DATA_DIR="${HOME}/.local/share/disk"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }

_version() { echo "disk v2.0.0"; }

_help() {
    echo "Disk v2.0.0 — sysops toolkit"
    echo ""
    echo "Usage: disk <command> [args]"
    echo ""
    echo "Commands:"
    echo "  scan               Scan"
...
_search() {
    local term="${1:?Usage: disk search <term>}"
...
case "${1:-help}" in
    scan)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/scan.log" 2>/dev/null || echo "  No entries yet. Use: disk scan <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/scan.log"
            local total=$(wc -l < "$DATA_DIR/scan.log")
            echo "  [Disk] scan: $input"
    ...
    monitor)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/monitor.log" 2>/dev/null || echo "  No entries yet. Use: disk monitor <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/monitor.log"
            local total=$(wc -l < "$DATA_DIR/monitor.log")
            echo "  [Disk] monitor: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: disk report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Disk] report: $input"
    ...
    alert)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/alert.log" 2>/dev/null || echo "  No entries yet. Use: disk alert <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/alert.log"
            local total=$(wc -l < "$DATA_DIR/alert.log")
            echo "  [Disk] alert: $input"
    ...
    top)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/top.log" 2>/dev/null || echo "  No entries yet. Use: disk top <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
    ...
    usage)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/usage.log" 2>/dev/null || echo "  No entries yet. Use: disk usage <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/usage.log"
            local total=$(wc -l < "$DATA_DIR/usage.log")
            echo "  [Disk] usage: $input"
    ...
    check)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/check.log" 2>/dev/null || echo "  No entries yet. Use: disk check <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/check.log"
            local total=$(wc -l < "$DATA_DIR/check.log")
            echo "  [Disk] check: $input"
    ...
    fix)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/fix.log" 2>/dev/null || echo "  No entries yet. Use: disk fix <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
    ...
    cleanup)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/cleanup.log" 2>/dev/null || echo "  No entries yet. Use: disk cleanup <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/cleanup.log"
            local total=$(wc -l < "$DATA_DIR/cleanup.log")
            echo "  [Disk] cleanup: $input"
    ...
    backup)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/backup.log" 2>/dev/null || echo "  No entries yet. Use: disk backup <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/backup.log"
            local total=$(wc -l < "$DATA_DIR/backup.log")
            echo "  [Disk] backup: $input"
    ...
    restore)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/restore.log" 2>/dev/null || echo "  No entries yet. Use: disk restore <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/restore.log"
            local total=$(wc -l < "$DATA_DIR/restore.log")
            echo "  [Disk] restore: $input"
    ...
    log)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/log.log" 2>/dev/null || echo "  No entries yet. Use: disk log <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
    ...
    benchmark)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/benchmark.log" 2>/dev/null || echo "  No entries yet. Use: disk benchmark <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/benchmark.log"
            local total=$(wc -l < "$DATA_DIR/benchmark.log")
            echo "  [Disk] benchmark: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: disk compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [Disk] compare: $input"
    ...
    *)
        ...
        echo "Run 'disk help' for available commands."
        exit 1
        ;;
esac