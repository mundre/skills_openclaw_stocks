---
name: omniscient
description: 整合认知套件、执行框架、系统控制三大能力层的全能助手；通过编排引擎自由组合原子操作完成复杂任务；当用户需要认知分析/代码生成/窗口管理/进程控制/硬件调节/串口通信/物联网控制/图形界面自动化时触发。
dependency:
  python:
    - pyserial>=3.0
    - requests>=2.25.0
...
通过专用 Python 脚本统一控制 Linux 桌面环境、系统硬件、串口设备和物联网平台。

### 运行环境

- **Python**: 标准 Python 3.x 环境
- **脚本目录**: `scripts/`
- **执行模式**: 始终使用命令执行工具运行脚本
- **系统依赖**:
  - `wmctrl` 或 `xdotool`: 窗口管理
  - `amixer`/`pactl`: 音量控制
  - `xbacklight`/`ddcutil`: 亮度控制
  - `xrandr`: 显示器控制
  - `scrot`/`gnome-screenshot`: 截图
  - `nmcli`/`ip`: 网络管理
  - `lsusb`: USB 设备

安装系统依赖（Ubuntu/Debian）:
```bash
sudo apt install wmctrl xdotool amixer xbacklight xrandr scrot nmcli network-manager lsusb
```

安装 Python 依赖:
```bash
pip install pyautogui pillow pyserial requests pytesseract
```
...
1. **破坏性操作必须用户确认**（关机、重启、杀进程、关闭窗口、禁用网卡）
2. **先查询再操作**（先 `list` 再 `close`，先 `list --name` 再 `kill`）
3. **电源操作需警告**（关机/重启/挂起/休眠前必须确认）
...
**窗口管理**:
- "关闭Firefox" → `python window_manager.py list` → 找到 Firefox → `python window_manager.py close --title "Firefox"`
- "把终端调到前台" → `python window_manager.py activate --title "Terminal"`

**进程管理**:
- "启动VS Code" → `python process_manager.py start "code"`
- "关掉所有gedit" → `python process_manager.py kill --name gedit`

**硬件控制**:
- "把音量调到50" → `python hardware_controller.py volume set --level 50`
- "锁屏" → `python hardware_controller.py power lock`
- "扫描WiFi" → `python hardware_controller.py network wifi`

**串口通信**:
- "有哪些串口" → `python serial_comm.py list`
- "给Arduino发指令开灯" → `python serial_comm.py send --port /dev/ttyUSB0 --data "LED_ON"`

**物联网控制**:
- "打开客厅灯" → `python iot_controller.py homeassistant --url ... --token ... on --entity-id light.living_room`

**图形界面自动化**:
- "截图" → `python gui_controller.py screenshot full`
- "点击(500,300)" → `python gui_controller.py mouse click --x 500 --y 300`
- "输入Hello World" → `python gui_controller.py keyboard type --text "Hello World"`
- "识别屏幕上的文字" → `python gui_controller.py visual ocr`
