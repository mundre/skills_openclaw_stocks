...
Version: 1.1.0 - Enhanced data parsing and smart slug mapping

Key Changes:
- Removed MCP server integration (simplified to DefiLlama + Dune only)
- Removed local risk scoring (LLM will analyze risk)
...
- Added safe_numeric() for robust TVL/audit parsing
- Added smart slug mapping (aave → aave-v3)
- Fixed audit display for numeric/string formats
    ...
    try:
        ...
        for name, section in matches:
            ...
            contract_match = re.search(
                r"\*\*Contract\*\*:\s*(`?0x[a-fA-F0-9]{40}`?)", section
            )
            if contract_match:
...
def fetch_yields() -> Dict[str, Any]:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "Web3-Investor/1.0.0"})
...
def fetch_protocols() -> List[Dict[str, Any]]:
    ...
    try:
        req = urllib.request.Request(
            DEFILLAMA_PROTOCOLS, headers={"User-Agent": "Web3-Investor/1.0.0"}
        )
        with urllib.request.urlopen(req, timeout=30) as response:
            return json.loads(response.read().decode())
    except urllib.error.URLError as e:
...
def safe_numeric(value: Any, default: float = 0.0) -> float:
    ...
    - str: parse numeric string (e.g., "2" -> 2.0)
    - list: sum of numeric elements (for TVL arrays)
    - dict: extract 'totalLiquidityUSD' or sum values
    """
    if value is None:
        return default
    ...
    if isinstance(value, list):
        ...
        for item in value:
            if isinstance(item, (int, float)):
                ...
                total += safe_numeric(
                    item.get("tvlUsd", item.get("totalLiquidityUSD", 0))
                )
        return total
    if isinstance(value, dict):
        # Try common TVL field names
        for key in ["totalLiquidityUSD", "tvlUsd", "tvl", "total"]:
            if key in value:
                return safe_numeric(value[key])
        return default
    return default
...
def safe_list_len(value: Any) -> int:
    """
    Safely get length of various types.

    Handles:
    - list: len(list)
    - dict: count of keys
    - str: 1 if non-empty (treat as single item)
    - int/float: 1 if non-zero (treat as single audit count)
    """
    if value is None:
        return 0
    if isinstance(value, list):
...
# Smart slug mapping for common protocol name variations
SLUG_MAPPING = {
    "aave": "aave-v3",
    "aave v2": "aave-v2",
    "aave v3": "aave-v3",
...
def normalize_slug(slug: str) -> str:
    """Normalize protocol slug to standard format."""
    if not slug:
        return slug
    slug_lower = slug.lower().strip()
    return SLUG_MAPPING.get(slug_lower, slug)
...
def extract_actionable_addresses(
    pool: Dict[str, Any], protocol: Dict[str, Any], registry: Dict[str, Dict[str, Any]]
) -> Dict[str, Any]:
    """
    ...
    result["underlying_token_addresses"] = [
        addr for addr in underlying if isinstance(addr, str) and addr.startswith("0x")
    ]

    # Reward tokens
    rewards = safe_list(safe_get(pool, "rewardTokens"))
    result["reward_token_addresses"] = [
        addr for addr in rewards if isinstance(addr, str) and addr.startswith("0x")
    ]
    ...
    result["has_actionable_address"] = bool(
        result["deposit_contract_candidates"] or result["underlying_token_addresses"]
    )
...
def collect_risk_signals(
    ...
    Args:
        ...
        protocol: Protocol data from DefiLlama
        registry: Static protocol registry
        total_tvl: Pre-calculated TVL (avoids double parsing)
    """
    protocol_name = safe_get(pool, "project", "").lower()
    registry_entry = registry.get(protocol_name, {})
    ...
    # Handle audits - can be list, int, or string like "2"
    audits_raw = safe_get(protocol, "audits")
    audit_count = safe_list_len(audits_raw)
    has_audit = audit_count > 0

    # Format audit info for display
    if isinstance(audits_raw, str) and audits_raw.isdigit():
        audit_display = f"{audits_raw} audits"
    elif isinstance(audits_raw, (int, float)) and audits_raw > 0:
        audit_display = f"{int(audits_raw)} audits"
    elif isinstance(audits_raw, list) and len(audits_raw) > 0:
        audit_display = f"{len(audits_raw)} audits"
    else:
        audit_display = "No audits"

    # Use provided TVL or calculate from pool
    pool_tvl = (
        total_tvl if total_tvl is not None else safe_numeric(safe_get(pool, "tvlUsd"))
    )
    protocol_tvl = safe_numeric(safe_get(protocol, "tvl"))
    ...
    return {
        ...
        "apy_composition": "base+rewards"
        if safe_get(pool, "apyReward")
        else "base_only",
        # Asset type signals
        ...
        # Investment preference signals
        "reward_type": detect_reward_type(pool),
        "has_il_risk": detect_il_risk(pool, category),
        "underlying_type": detect_underlying_type(pool, category, symbol),
        # Governance signals
...
def check_execution_readiness_inline(network: str = "base") -> Dict[str, Any]:
    ...
    try:
        req = urllib.request.Request(
            ...
            headers={"User-Agent": "Web3-Investor/1.0.0"},
            method="GET",
        )
...
def find_opportunities(
    ...
    # Build protocol lookup with smart slug mapping
    protocol_map = {}
    for p in protocols_data:
        slug = safe_get(p, "slug", safe_get(p, "name", "")).lower()
        protocol_map[slug] = p
        protocol_map[safe_get(p, "name", "").lower()] = p
        # Also add normalized slug
        normalized = normalize_slug(slug)
        if normalized != slug:
            protocol_map[normalized] = p
    ...
    for pool in yields_data.get("data", []):
        ...
        if (
            normalized_chain.lower() not in pool_chain
            and chain.lower() not in pool_chain
        ):
            continue
        ...
        # Filter by TVL - use safe_numeric for robust handling
        tvl = safe_numeric(safe_get(pool, "tvlUsd"))
        if tvl < min_tvl:
            continue

        # Get protocol info with smart slug lookup
        ...
        if not protocol:
            normalized_slug = normalize_slug(protocol_name.lower())
            protocol = protocol_map.get(normalized_slug, {})
        ...
        # Add risk signals for LLM analysis - pass calculated TVL
        if include_risk_signals:
            opp["risk_signals"] = collect_risk_signals(
                pool, protocol, registry, total_tvl=tvl
            )
    ...
    # Attach execution readiness to each opportunity
    ...
    print(
        f"📊 Found {len(opportunities)} opportunities from DefiLlama", file=sys.stderr
    )
    ...
    for i, opp in enumerate(opportunities, 1):
        ...
        if actionable.get("has_actionable_address"):
            ...
            if actionable.get("primary_contract"):
                lines.append(
                    f"| Primary Contract | `{actionable['primary_contract']}` |"
                )
        else:
            lines.append(f"| Actionable | ⚠️ Needs address lookup |")

        lines.extend(["", f"**Pool**: `{opp['pool']}`", f"**Link**: {opp['url']}", ""])
        ...
        if risk:
            lines.extend(
                [
                    f"**Risk Signals**:",
                    f"- Audited: {risk.get('audit_display', 'No audits')}",
...
def main():
    parser = argparse.ArgumentParser(
        description="Find DeFi investment opportunities (v1.0.0 - DefiLlama + Dune)",
...
Examples:
  ...
  python find_opportunities.py --allow-high-apy --limit 50
    ...
    parser.add_argument(
        "--max-apy",
        type=float,
        default=MAX_SAFE_APY_DEFAULT,
        help=f"Maximum APY %% (default {MAX_SAFE_APY_DEFAULT}%)",
    )
    parser.add_argument(
        "--allow-high-apy", action="store_true", help="Allow APY > 100%% (high risk)"
    )
    ...
    if args.allow_high_apy:
        print(
            "⚠️ Warning: --allow-high-apy enabled, may include high-risk projects",
            file=sys.stderr,
        )
