...
Optional:
  ...
  --repo <owner/name>    GitHub repository (required with --create-issues)
...
Environment:
  GITHUB_TOKEN           GitHub token with issue write permissions
`;
...
function normalizeSignals(payload, source) {
  ...
  if (source === 'sentry' && Array.isArray(payload.issues)) {
...
async function main() {
  ...
  const signals = dedupeSignals(
    rankSignals([
      ...normalizeSignals(analytics, 'analytics'),
      ...normalizeSignals(revenuecat, 'revenuecat'),
      ...normalizeSignals(sentry, 'sentry'),
      ...normalizeSignals(feedback, 'feedback'),
    ]),
  ...
  if (!args.createIssues) {
    process.stdout.write('Dry run only. Re-run with --create-issues --repo <owner/name> to create GitHub issues.\n');
    return;
  }

  if (!args.repo) {
    throw new Error('Missing --repo <owner/name> while using --create-issues.');
  }
  const token = process.env.GITHUB_TOKEN;
  if (!token) {
    throw new Error('Missing GITHUB_TOKEN environment variable.');
  }
  const defaultBranch = args.uploadCharts ? await getRepoDefaultBranch(args.repo, token) : null;
