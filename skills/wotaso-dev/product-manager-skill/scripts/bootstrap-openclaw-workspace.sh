...
#
# Typical layout after `clawhub install product-manager-skill`:
#   <workspace>/skills/product-manager-skill/scripts/*.mjs
# This script creates:
...
if [[ "$(basename "${SKILL_ROOT}")" != "product-manager-skill" ]]; then
  echo "bootstrap-openclaw-workspace.sh: expected to live under skills/product-manager-skill/scripts/ (ClawHub install)." >&2
...
echo "Copied product-manager-skill runtime into workspace:"
