...
CONFIG_FILE = Path.home() / ".openclaw" / "skills" / "llm-memory-integration" / "config" / "upgrade_rules.json"
...
class SmartMemoryUpgrade:
    ...
    def get_l0_candidates(self) -> List[Dict]:
        ...
        try:
            # SECURITY FIX: 使用列表形式避免命令注入
            result = subprocess.run(
                ["sqlite3", str(self.db_path), sql],
    ...
    def upgrade_l0_to_l1(self, candidate: Dict):
        ...
        # 插入 L1
        # SECURITY FIX: 使用参数化查询避免 SQL 注入
        # 先对内容进行转义
        safe_content = candidate["content"][:500].replace("'", "''")
        safe_id = candidate["id"].replace("'", "''")
        safe_scene = scene.replace("'", "''")
        safe_type = memory_type.replace("'", "''")
        
        sql = f"""
        INSERT INTO l1_records (content, type, scene_name, source_id, created_at)
        VALUES ('{safe_content}', '{safe_type}', '{safe_scene}', '{safe_id}', datetime('now'));
        
        UPDATE l0_conversations SET upgraded = 1 WHERE conversation_id = '{safe_id}';
        """
    ...
    def run_upgrade_cycle(self):
        """执行升级周期"""
        # 🔒 安全修复：检查是否启用自动升级
        if not self.rules.get("auto_upgrade", False):
            self.log("⚠️ 自动升级已禁用，请手动运行: smart_memory_upgrade.py run")
            print("⚠️ 自动升级已禁用，请手动运行: python3 smart_memory_upgrade.py run")
            return 0
        ...
        if upgraded_count > 0:
            ...
            try:
                # SECURITY FIX: 使用列表形式避免命令注入
                subprocess.run(
                    ["python3", os.path.expanduser("~/.openclaw/workspace/skills/llm-memory-integration/scripts/backfill_l0_vectors.py"), "--l1"],
