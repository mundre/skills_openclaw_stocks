...
description: |
DINGs 托管式 AI 电话助手 - 中国、日本餐厅全自动 AI 电话预订技能，支持餐厅搜索+AI 电话自动预订一体化服务，只要你告诉我用餐需求（或需搜索的餐厅信息/大众点评链接），即可帮你完成餐厅搜索、自动拨打餐厅电话完成预订的全流程任务。
当用户提到以下意图时使用此技能:
...
支持:中国、日本餐厅搜索（支持名称/大众点评链接）、AI 电话自动外呼预订、预订结果回调通知、预订状态查询、通话记录查看、电子凭证生成。
元数据
{
    "openclaw": {
        "requires": {
...
支持名称/大众点评链接，传入城市/国家code，返回餐厅列表（含dings_id）；城市需按支持列表填写
...
方法2：搜索餐厅后预订
1. 助手询问用户需搜索的餐厅名称/大众点评链接、所在城市（按支持列表）、所属国家（中国/日本）；
...
接口说明：根据餐厅名称/大众点评链接、城市（按支持列表填写）、国家code搜索中国/日本餐厅，返回餐厅列表（含dings_id、可预订状态等核心信息），为后续预订提供餐厅数据支撑。
