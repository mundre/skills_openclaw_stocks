...
在执行评测前，**先读取 `~/.openclaw/openclaw.json` 获取默认模型配置**，然后展示以下信息等待用户确认：

> 📊 **BenchClaw 评测即将开始**
>
> - ⏱️ **预计耗时**：10-60 分钟（取决于模型速度和网络状况）
> - 💰 **Token 消耗**：约 2-3M tokens（会产生 API 费用，请确认预算充足）
> - 📋 **评测内容**：25 道题，涵盖能力、配置、硬件、权限、安全 5 大分类
> - ⚠️ **期间注意**：OpenClaw 仍可响应其他消息，但性能会有所下降
> - 🤖 **评测模型：`{agents.defaults.model.primary}`**（从 openclaw.json 读取）
> - ⚠️ 评测使用的是 openclaw.json 配置的默认模型，与你当前 session 无关。
>
> 最后一个问题：**是否在榜单上展示你的名字「{AI名字}」？**
> - 回复「**展示 / 开始 / 确认**」→ 以「{AI名字}」出现在榜单上，立即开始评测 🚀
> - 回复「**匿名**」→ 匿名参与榜单，立即开始评测 🚀
>
> （成绩和硬件配置都会上传，仅名字显示方式不同）

根据用户回复，写入 `temp/caller_info.txt` 并启动评测：

> **channel 和 target 从当前 inbound_meta 取，支持所有渠道（飞书/微信/元宝等）：**
> - `channel` = `inbound_meta.channel`（如 `feishu`、`openclaw-weixin`、`yuanbao` 等）
> - `target` = `inbound_meta.chat_id`（如飞书的 `user:ou_xxx`，微信的 `o9cq803G...@im.wechat`）

```bash
# caller_info.txt 说明：
# 评测进程（main.py）在后台运行，与当前 session 隔离。
# 此文件用于告知 main.py 评测完成后如何回调通知用户。
# 仅包含：渠道名、用户 chat_id、榜单展示名称（可选）。
# 文件在本机 temp/ 目录内，不上传至任何外部服务器。

# 用户回复「展示/开始/确认」时：
# agent_name：从 SOUL.md 第二行提取"你是XX"后面的名字
# 示例：SOUL.md 第二行为 "你是爱兔 - 安兔兔的AI助理" → 提取 "爱兔"
# 找不到名字时，使用 "OpenClaw Agent" 作为默认值
echo "channel=<inbound_meta.channel>" > scripts/../temp/caller_info.txt
echo "target=<inbound_meta.chat_id>" >> scripts/../temp/caller_info.txt
echo "agent_name=<从 SOUL.md 提取的AI名字>" >> scripts/../temp/caller_info.txt
...
启动后告知用户：
> ✅ 评测已启动，预计 10-60 分钟完成。**完成后会自动发消息通知你，无需等待。**

> 💡 **TUI 用户注意：** 如果你通过 TUI 或终端直接触发评测，**不要写 `caller_info.txt`**（或者不要执行上面的 `echo` 命令）。评测进度和结果会直接输出到终端（stdout），你可以查看终端日志获取进度。写入 `channel=webchat` 或其他内部渠道会导致通知发送失败。
...
🌐 网络：{如有 ping_ms 数据则展示，否则跳过此行}
{根据 ping_ms 评价：< 100ms ✅优秀 / 100-300ms 🟡一般 / 300-500ms 🟠较慢 / > 500ms 🔴过慢}
...
→ 网络延迟较高（{ping_ms}ms）：建议将服务器迁移到更接近 API 服务器的地区
