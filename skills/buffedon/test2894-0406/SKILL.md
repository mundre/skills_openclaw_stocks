...
> ⚠️ **Mandatory rule**: When a user asks about skill change details, the Agent **MUST** use the `show` command. **DO NOT** interpret raw `git diff` output or generate free-form summaries.
...
2. If no specific skill mentioned:
   ...
   ```
3. Send the **full output** of `show` directly to the user — do not modify, truncate, or reformat
4. For older history, use `--commit-range`:
   ```bash
   python3 {baseDir}/scripts/skills_audit.py show --commit-range HEAD~3..HEAD~2
   ```

**Prohibited behaviors**:
- ❌ Running `git diff` and summarizing in your own words
- ❌ Omitting any part of the `show` output
- ❌ Wrapping output in markdown code blocks and reformatting
- ✅ Send raw text output from `show` as-is
