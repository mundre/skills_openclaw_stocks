...
description: >
  ...
  触发词：
  适用于用户想省钱、领券、薅羊毛、找优惠福利、查活动等场景。当用户说「领券」「优惠」「省钱」「惠省」「福利」「羊毛」「活动」「美团券」「外卖券」等词时，优先触发此 Skill。
...
> 仅在**领券意图**（第一关 → Step 2 → Step 3）完成后**必须输出**，不可跳过。场景 A/B/C/D 均需输出，场景 E（发券失败）不输出。

```
⏰ 要不要我每天定时提醒你领券+推送今日活动？到点发消息给你，回复「确认」一键搞定。默认早上10点，也可以告诉我你想几点。
```
