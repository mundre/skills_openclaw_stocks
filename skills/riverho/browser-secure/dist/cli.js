...
import { startBrowser, closeBrowser, getBrowserStatus, suspendSession, resumeSession, getPage, reconnectToDaemon } from './browser/secure-session.js';
import { startDaemon, stopDaemon, printDaemonStatus, getDaemonStatus, isDaemonRunning, loadDaemonState } from './browser/daemon.js';
import { readAuditLog, rotateAuditLog } from './security/audit.js';
import { logLaunchError, logDaemonError, logCredentialError, logSessionError, readErrorLog, getErrorStats, clearErrorLog } from './utils/error-log.js';
import { loadConfig, getConfigPath, checkCredentialSource } from './config/loader.js';
...
program
    ...
    for (const e of entries) {
        ...
        if (e.stack) {
            ...
            if (firstLine)
                console.log(`   stack: ${firstLine.trim()}`);
        }
    }
...
program
    ...
    try {
        ...
        if (analysis.searchSelector)
            console.log(`  Search selector: ${analysis.searchSelector}`);
        ...
        if (playbook.discovery.search_url)
            console.log(`  Search URL: ${playbook.discovery.search_url}`);
        if (playbook.discovery.search_input_selector)
            console.log(`  Search input: ${playbook.discovery.search_input_selector}`);
        console.log(`  Access: login=${playbook.access.login_required}`);
        if (playbook.extraction.body_selector)
            console.log(`  Body selector: ${playbook.extraction.body_selector}`);
...
program
    ...
    try {
        ...
        if (result.capture.approval_chain) {
            ...
            if (auto)
                console.log(`   Approval: auto-approved (--yes)`);
        }
        rl.close();
        await closeBrowser();
    }
...
program
    ...
    .action(async (options) => {
    const { listCaptures, listSiteProfiles, exportToMarkdown, exportToJson } = await import('./research/scrapbook.js');
    if (options.sites) {
