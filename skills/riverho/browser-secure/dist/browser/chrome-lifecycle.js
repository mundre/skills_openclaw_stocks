...
function copyDirSync(src, dest) {
    ...
    for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
        if (EXCLUDED_COPY_FILES.has(entry.name))
            continue;
 ...
 */
export function copyChromeProfileToTemp(profileId) {
...
async function waitForChromeExit(maxWaitMs = 15000) {
...
async function promptQuit(profileName) {
    ...
    const question = '\n⚠️  Profile mode requires exclusive access to your Chrome user data dir.\n' +
        ...
        `    Profile: ${profileName}\n\n` +
        '    Continue? [y/N]: ';
    const answer = await new Promise(resolve => rl.question(question, ans => resolve(ans.trim())));
    rl.close();
    return /^y(es)?$/i.test(answer);
}
function quitChromeProcess() {
    const p = os.platform();
    if (p === 'darwin') {
    ...
    throw new Error(`Auto-quit for Chrome is not supported on ${p}. Close Chrome manually and retry.`);
}
export async function ensureChromeClosedForProfile(opts) {
    if (!isChromeRunning())
        return;
    if (opts.isUnattended && !opts.allowQuit) {
        throw new Error('Chrome is running and --profile requires exclusive access to its user data dir.\n' +
    ...
    if (!opts.isUnattended && !opts.allowQuit) {
        ...
        if (!ok) {
            throw new Error('Aborted: Chrome still running. Retry with Chrome closed, or omit --profile.');
        }
    }
    ...
    if (!exited) {
        throw new Error('Chrome did not exit within 15s. It may have an unsaved-changes dialog open. ' +
