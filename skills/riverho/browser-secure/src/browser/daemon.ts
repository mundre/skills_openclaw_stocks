...
export interface StartDaemonOptions {
  profileId?: string;
  // Required when Chrome is running and we're in unattended mode. In
  // interactive mode the helper prompts the user directly.
  closeChrome?: boolean;
  isUnattended?: boolean;
}

export async function startDaemon(
  optsOrProfile?: string | StartDaemonOptions
): Promise<{ state: DaemonState }> {
  const opts: StartDaemonOptions =
    typeof optsOrProfile === 'string' ? { profileId: optsOrProfile } : (optsOrProfile ?? {});
  ...
  if (!fs.existsSync(realProfileDir)) {
    throw new Error(
      `Chrome profile "${profile}" does not exist.\n` +
