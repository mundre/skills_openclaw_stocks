...
async function trySelector(page: Page, selectors: string[]): Promise<string | null> {
...
async function extractBySelectors(page: Page): Promise<ExtractedContent | null> {
...
async function extractByDensity(page: Page): Promise<ExtractedContent | null> {
...
export async function extractWithCustomSelectors(
  page: Page,
  selectors: { title?: string; body?: string; author?: string; date?: string }
): Promise<ExtractedContent> {
  const selList: string[] = [];
  if (selectors.body) selList.push(selectors.body);
  const body = await trySelector(page, selList);

  const titleSel: string[] = [];
  if (selectors.title) titleSel.push(selectors.title);
  const title = await trySelector(page, titleSel);

  const authorSel: string[] = [];
  if (selectors.author) authorSel.push(selectors.author);
  const author = await trySelector(page, authorSel);

  const dateSel: string[] = [];
  if (selectors.date) dateSel.push(selectors.date);
