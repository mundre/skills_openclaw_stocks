...
export async function analyzeSite(page: Page, url: string): Promise<SiteAnalysis> {
  ...
  const loginInfo = await page.evaluate(() => {
    ...
    const loginLinks = Array.from(document.querySelectorAll('a')).filter(a =>
      /sign.in|log.in|login|authenticate|member/i.test(a.textContent || '')
    );
    return {
  ...
  if (searchInfo.selector) analysis.searchSelector = searchInfo.selector;
    ...
    for (const sel of selectors) {
      const links = Array.from(document.querySelectorAll(sel))
        ...
        .map(a => ({
          ...
          url: (a as HTMLAnchorElement).getAttribute('href') || '',
        }));
...
export function buildPlaybook(analysis: SiteAnalysis, loginRequired: boolean): SitePlaybook {
  return {
    discovery: {
