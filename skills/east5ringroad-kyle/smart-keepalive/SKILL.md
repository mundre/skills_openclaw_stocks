...
**Description:** 定时抓取 RSS 热点并通过 OpenClaw 自动发送「保活简报 keepalive」（双关 keepalive + 资讯简报），支持 `openclaw agent` 文案润色、可选作息附录，以及 launchd/cron 的定时部署辅助。
...
- **脚本只做**：拉 RSS → 填模板变量 → `openclaw agent` 生成正文 → `openclaw message send`。换 RSS URL、权重常量或管线行为时改 Python，并**改本文对应表/段落**。
...
3. **正文**：有序列表；天气行置底；作息关怀由脚本改写为「问候与关怀同行 → 空行 → 单独栏目行 → 列表」，见 `smart-keepalive.py` 中 `insert_wellness_after_greeting`。
...
1. **`build_keepalive_brief()`**：只组装 RSS 原始行（可含链接），不做面向用户的「业务级」润色。内部顺序概览：先算**综合乘数**（时段 × 轮换 × 天气 × 节假日 × 跨日新鲜度）→ **盲盒**（`KEEPALIVE_BRIEF_CHAOS`，命中则**不参与**上述乘数，在今日仍可用源上均匀随机试拉，成功即返回）→ 否则 **基准权重 × 综合乘数** 后加权随机 → 主抽中源失败则**同轮乱序**试其余源 → **热点兜底**（多轮打乱顺序重试；轮数上限 `KEEPALIVE_BRIEF_HOT_FALLBACK_ROUNDS`，默认 `4`）。去重与 `tag_meta` 见下「简报素材随机」。
...
6. **发送**：`openclaw message send`；遇 `Unknown channel` 重试一次。
...
- **首次 `--setup` 会要求填写天气城市**，写入 `config.json` 的 `weatherCity`（可被环境变量覆盖）。
- **定时任务找不到 `openclaw`**：设 `OPENCLAW_BIN` 或按 `--doctor` 修 PATH。
...
- **时段权重（默认开启）**：同一时段内仍是**加权随机**（非固定顺序）。在**尚未被今日用过**的源上，先按基准权重再乘本地时段系数，并与下述「轮换 / 天气 / 节假日 / 新鲜度」**连乘**后归一化抽样；另有 **盲盒**与 **约 10% 兜底**（见上流程）。关闭：`KEEPALIVE_BRIEF_TIME_WEIGHTS=0`。
...
- **天气**：雨雪等恶劣天气时略提高新闻/中文资讯源（法广、知乎、南周、新华社等），压低「每日环球视野」（需能拉到 `weatherCity` 的天气接口；脚本用现象词匹配 **中文**「雨/雪/雹/雷阵…」及 **英文** rain/snow/sleet 等）。关闭：`KEEPALIVE_BRIEF_WEATHER_BIAS=0`。
...
**加权池**（与脚本 `base_weighted` / `BRIEF_TAG_KEYS` 一致）：基准权重之和 **0.90**，余量 **0.10** 为热点兜底（`BRIEF_FALLBACK_WEIGHT`）；再与时段/轮换/天气/节假日/新鲜度等**连乘**后，在**当日仍可用**源上归一化加权随机。

| 基准权重 | 数据源 | 日志 `theme_tag` |
|---------|--------|-------------------|
| 0.10 | RSSHub `bilibili/weekly` | `bilibili-weekly` |
| 0.10 | 知乎日报（AnyFeeder） | `zhihu-daily` |
| 0.10 | 南方周末推荐（AnyFeeder `infzm/recommends`） | `infzm-recommends` |
| 0.10 | 每日环球视野（AnyFeeder） | `idaily-today` |
| 0.10 | 少数派 RSS（直连） | `sspai-feed` |
| 0.10 | 果壳科学人（AnyFeeder `guokr/scientific`） | `guokr-scientific` |
| 0.10 | 法广中文网（AnyFeeder `rfi/cn`，法国国际广播电台中文） | `rfi-cn` |
| 0.10 | 新华社新闻·新华网（AnyFeeder `newscn/whxw`） | `xinhua-whxw` |
| 0.10 | 36氪（直连） | `36kr-feed` |
...
**热点兜底**（`run_theme(4)` / `run_theme_hot_fallback`）：多路源（B 站每周必看 / 南方周末推荐 / 新华社 / 法广中文网 / 果壳科学人 / 36氪 / 虎嗅）；外层轮数上限由 `KEEPALIVE_BRIEF_HOT_FALLBACK_ROUNDS` 控制（默认 `4`，允许 `1`–`32`），每轮随机顺序（与 `_brief_time_bucket` 偏好组合），先跳过当日已用源再全员重试；合并正文后洗牌，最多取 5 条。
...
RSSHub 仅用于 `bilibili/weekly`；其余新闻/科技源多为 AnyFeeder `plink` 直链。
...
| `KEEPALIVE_BRIEF_WEATHER_BIAS` | `1`（默认）雨雪天微调国内/环球视野权重；`0` 关闭 |
...
| `KEEPALIVE_WEATHER_CITY` | 天气城市（覆盖 `config.json.weatherCity`） |
...
**Smart Keepalive** sends periodic messages via `openclaw message send`. **Copy and layout live in `prompts/rewrite-main.md`** (fixed **保活简报 keepalive** header, time-based greeting, RSS body); **`prompts/wellness.md`** covers optional wellness lines. The script fetches RSS via a weighted pool (time slot, 4h rotation, weather/holiday/freshness tweaks, optional chaos draw, daily dedup, multi-retry hot fallback), fills templates, runs `openclaw agent`, and sends. Edit those markdown files first when changing filters or tone.
...
3. 手动试发：`openclaw message send --message "ping"`（按需加 `--channel` / `--target`），确认通道可用。
4. RSS / 天气：若简报里常出现「本轮暂无可用条目」或天气缺失，检查网络、RSS 源与 `KEEPALIVE_RSSHUB_BASES`、天气接口是否偶发失败（脚本已做重试与兜底，不必夸大成「服务宕机」）。
...
When the user asks about “keepalive” health: clarify it means the **scheduled pipeline** (RSS → agent → `openclaw message send`), not full infra monitoring. Prefer `--doctor`, `smart-keepalive.log`, then a manual `openclaw message send` test. Do not invent incidents not present in logs.
