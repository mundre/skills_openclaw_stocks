...
function sortLedgerRows(rows) {
  return [...rows].sort((left, right) => {
    return compareNatural(left['地块名'], right['地块名'])
      ...
      || compareFloor(left['自然楼层'], right['自然楼层'])
...
function getLatestProjectSnapshot(rows, projectName) {
  ...
  if (!projectRows.length) {
    return { signedUnits: 0, averagePrice: 0, updatedAt: null };
  }
  const sortedProjectRows = sortLedgerRows(normalizeLedgerRows(projectRows));
  const latestRow = sortedProjectRows[sortedProjectRows.length - 1];
...
function roomKeyCandidates({ projectName, buildingName, naturalFloor, roomNo, buildingId }) {
  ...
  if (buildingId) {
    names.add(`building-${buildingId}`);
  }
  return [...names].filter(Boolean).map(name => roomKey({
    projectName,
    buildingName: name,
    naturalFloor,
    roomNo
...
function parseProjectField(text, label) {
...
function parseProjectSummary(html) {
  ...
  const projectName = parseProjectField(text, '项目名称');
  const salePermitNumber = parseProjectField(text, '预售许可证编号');
  const statMatch = text.match(/期房签约统计[\s\S]*?住宅\s+(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)/);
  if (!statMatch) {
    throw new Error('未能从项目详情页解析“期房签约统计”');
  }
  return {
    projectName,
    salePermitNumber,
...
async function fetchAllBuildingPageUrls(projectHtml, projectUrl) {
  ...
  for (const url of extractBuildingUrls(firstPageHtml, moreUrl)) {
    urls.add(url);
  }
  ...
  for (let currentPage = 2; currentPage <= totalPages; currentPage += 1) {
    ...
    for (const url of extractBuildingUrls(pageHtml, moreUrl)) {
      urls.add(url);
    }
  }
...
function parseSignedRoomsFromBuildingPage(html, buildingUrl) {
  ...
  while ((rowMatch = rowRegex.exec(tableHtml)) !== null) {
    ...
    const saleFloor = stripTags(rowMatch[2]);
    ...
    while ((roomMatch = roomRegex.exec(roomCellHtml)) !== null) {
      ...
      if (!SIGNED_LIKE_STATUSES.has(status)) continue;
...
function normalizeProjectEntry(project = {}) {
  ...
  return {
    name: normalizedName,
    urls,
    updatedAt: normalizeWhitespace(project.updatedAt) || nowShanghaiString()
  };
}
...
function mergeProjectEntries(projects = []) {
  ...
  for (const project of projects) {
    ...
    if (compareNatural(existing.updatedAt, normalized.updatedAt) < 0) {
...
function saveConfig(configPath, config) {
  writeJson(configPath, {
    ...config,
    projects: mergeProjectEntries(config.projects)
  });
}
...
function computeEstimatedAveragePrice(previousSignedUnits, previousAveragePrice, currentSignedUnits, currentAveragePrice) {
  const delta = currentSignedUnits - previousSignedUnits;
  if (delta <= 0) return null;
  const value = ((currentSignedUnits * currentAveragePrice) - (previousSignedUnits * previousAveragePrice)) / delta;
  return round2(value);
}
...
function matrixRowToLedgerRow(values = []) {
  const padded = [...values];
  while (padded.length < LEDGER_COLUMNS.length) padded.push('');
  const row = {};
  LEDGER_COLUMNS.forEach((header, index) => {
    row[header] = normalizeWhitespace(padded[index]);
  });
  return row;
}
...
function isHeaderRow(values = []) {
  return LEDGER_COLUMNS.every((header, index) => normalizeWhitespace(values[index]) === header);
}
...
async function getTenantAccessToken(appId, appSecret) {
  if (!appId || !appSecret) {
    throw new Error('缺少飞书 app_id 或 app_secret，请在配置文件、环境变量或命令参数中提供');
  }
...
function formatRowsAsMobileText(rows) {
  ...
  return groups.map(({ name, rows: groupedRows }) => {
    const firstRow = groupedRows[0] || {};
    const signedCount = firstRow['项目已签约套数'] ?? '';
    const averagePrice = firstRow['项目成交均价'] ?? '';
    const header = [
      `=== ${name}（${groupedRows.length} 套） ===`,
      `项目签约：${signedCount}`,
      `项目均价：${averagePrice}`
    ].join('\\n');
    const body = groupedRows.map((row, index) => `【${name} - 新增 ${index + 1}】\\n${formatRowAsMobileText(row)}`).join('\\n\\n');
    return `${header}\\n${body}`;
  }).join('\\n\\n');
}
...
async function sendNewRowsNotification(accessToken, notifyUserOpenId, rows) {
  if (!notifyUserOpenId || !rows.length) {
    return { sent: false, messageCount: 0 };
  }
  const sortedRows = sortLedgerRows(normalizeLedgerRows(rows));
  const rowChunks = chunkNotificationRows(sortedRows);
  for (const [index, chunk] of rowChunks.entries()) {
    const title = rowChunks.length === 1
      ? `北京住建委签约更新：新增 ${rows.length} 行\n`
      : `北京住建委签约更新：新增 ${rows.length} 行（第 ${index + 1}/${rowChunks.length} 条消息）\n`;
...
function pickTargetSheet(sheets, preferredSheetId, preferredSheetTitle) {
  if (!sheets.length) {
    throw new Error('目标飞书表格中没有可用工作表');
  }
  if (preferredSheetId) {
    const matched = sheets.find(sheet => sheet.sheet_id === preferredSheetId);
    if (!matched) {
      throw new Error(`未找到指定 sheet_id: ${preferredSheetId}`);
    }
    return matched;
  }
  if (preferredSheetTitle) {
    ...
    if (!matched) {
      throw new Error(`未找到指定工作表标题: ${preferredSheetTitle}`);
    }
    return matched;
  }
...
async function readSheetMatrix(spreadsheetToken, sheetId, accessToken) {
  ...
  const json = await fetchJson(url, {
    method: 'GET',
    headers: {
      ...buildFeishuAuthHeaders(accessToken),
...
async function writeSheetRange(spreadsheetToken, accessToken, range, values) {
  ...
  if (values.length > MAX_WRITE_ROWS) {
    throw new Error(`单次写入行数超过限制 ${MAX_WRITE_ROWS}`);
  }
  if (values[0].length > MAX_WRITE_COLUMNS) {
    throw new Error(`单次写入列数超过限制 ${MAX_WRITE_COLUMNS}`);
  }
...
async function rewriteSheetSorted(spreadsheetToken, accessToken, sheetId, allRows) {
  ...
  for (let offset = 0; offset < matrix.length; offset += MAX_WRITE_ROWS) {
    ...
    const range = `${sheetId}!A${startRow}:H${endRow}`;
...
function parseLedgerRowsFromSheet(matrix) {
  ...
  if (!isHeaderRow(matrix[0])) {
    throw new Error('飞书表格首行不是预期表头，请先清空或修正表头后再同步');
  }
  const rows = matrix.slice(1)
    .filter(row => row.some(cell => normalizeWhitespace(cell)))
    .map(matrixRowToLedgerRow);
  return { rows, hasHeader: true };
}

async function ensureSheetHeader(spreadsheetToken, accessToken, sheetId, matrix) {
  if (matrix.length && isHeaderRow(matrix[0])) return;
  if (matrix.length) {
    throw new Error('飞书表格首行不是预期表头，且已存在数据，已停止以避免破坏历史数据');
  }
  await writeSheetRange(spreadsheetToken, accessToken, `${sheetId}!A1:H1`, [LEDGER_COLUMNS]);
}

async function syncOneProject(project, existingRows, existingKeys) {
  const sourceUrls = getProjectUrls(project);
  if (!sourceUrls.length) {
    throw new Error(`项目 ${project.name || '未命名地块'} 没有可用链接`);
  }
  ...
  if (!sourceSummaries.length) {
    throw new Error(`该地块下 ${sourceUrls.length} 个链接都无法解析；最后错误：${skippedSources[skippedSources.length - 1]?.reason || '未知错误'}`);
  }

  const allSignedRooms = [];
  for (const buildingUrl of buildingUrlSet) {
    const buildingHtml = await fetchText(buildingUrl);
    const { rooms } = parseSignedRoomsFromBuildingPage(buildingHtml, buildingUrl);
    allSignedRooms.push(...rooms);
  }

  const uniqueSignedRooms = [];
  const seen = new Set();
  for (const room of allSignedRooms) {
    const key = roomKey({
      projectName,
  ...
  const weightedAverageSum = sourceSummaries.reduce((sum, summary) => {
    return sum + (Number(summary.signedUnits || 0) * Number(summary.averagePrice || 0));
  }, 0);
  const currentAveragePrice = currentSignedUnits > 0 ? round2(weightedAverageSum / currentSignedUnits) : 0;
  ...
  const estimatedAveragePrice = computeEstimatedAveragePrice(
    previousSnapshot.signedUnits,
    previousSnapshot.averagePrice,
    currentSignedUnits,
    currentAveragePrice
  );

  const newRooms = uniqueSignedRooms.filter(room => !hasExistingRoomKey(existingKeys, {
    projectName,
    buildingName: room.buildingName,
    naturalFloor: room.naturalFloor,
    roomNo: room.roomNo,
  ...
  return {
    ...
    trackedSignedRooms: uniqueSignedRooms.length,
    ...
    warnings: [
      ...
      ...(deltaSignedUnits < 0 ? [`项目总签约套数从 ${previousSnapshot.signedUnits} 降到 ${currentSignedUnits}，未追加新行。`] : []),
      ...(deltaSignedUnits > 0 && estimatedAveragePrice === null ? ['签约套数有增量，但未能计算估计成交均价。'] : []),
      ...(deltaSignedUnits > 0 && deltaSignedUnits !== newRooms.length ? [`统计增量为 ${deltaSignedUnits} 套，但本次在楼盘表中发现 ${newRooms.length} 套新房号。`] : []),
      ...(deltaSignedUnits <= 0 && newRooms.length > 0 ? ['楼盘表发现了历史中不存在的房号，但项目签约套数没有增加，已跳过以避免错误估价。'] : [])
    ]
  };
}
...
可选参数:
  ...
  --notify-user-open-id <open_id>  有新增时通知的飞书私聊 open_id
  --json                 以 JSON 输出同步结果
...
function requireFeishuConfig(feishuConfig) {
  if (!feishuConfig.spreadsheetToken) {
    throw new Error('缺少飞书表格链接或 spreadsheet token');
  }
...
async function main() {
  ...
  if (command === 'add') {
    if (!options.name || !options.url) {
      throw new Error('add 命令需要同时提供 --name 和 --url');
    }
  ...
  if (command === 'remove') {
    if (!options.name) {
      throw new Error('remove 命令需要提供 --name');
    }
    const removed = removeProject(config, options.name);
    if (!removed) {
      throw new Error(`未找到项目: ${options.name}`);
    }
    saveConfig(configPath, config);
  ...
  if (command === 'list') {
    ...
    for (const [index, project] of config.projects.entries()) {
      ...
      for (const [urlIndex, url] of getProjectUrls(project).entries()) {
  ...
  let projectsToSync = [];
  if (options.name && options.url) {
    projectsToSync = [{ name: options.name, url: options.url }];
  } else if (options.name) {
    const matched = config.projects.find(project => project.name === options.name);
    if (!matched) {
      throw new Error(`配置中不存在项目: ${options.name}`);
    }
  ...
  for (const project of projectsToSync) {
    const report = await syncOneProject(project, existingRows, existingKeys);
    reports.push(report);
  ...
  if (rowsToAppend.length) {
    await appendSheetRows(
      feishuConfig.spreadsheetToken,
      accessToken,
      targetSheet.sheet_id,
      rowsToAppend.map(ledgerRowToMatrixRow)
    );
    ...
    notification = await sendNewRowsNotification(accessToken, feishuConfig.notifyUserOpenId, rowsToAppend);
  }
  ...
  if (rowsToAppend.length) {
    if (feishuConfig.notifyUserOpenId) {
      console.log(`已发送飞书通知: ${notification.sent ? `是（${notification.messageCount} 条消息）` : '否'}`);
    } else {
      console.log('未发送飞书通知: 未配置 notify_user_open_id');
    }
  }
  console.log('');
  ...
  for (const report of reports) {
    ...
    if (report.salePermitNumber) {
      console.log(`预售证: ${report.salePermitNumber}`);
    }
