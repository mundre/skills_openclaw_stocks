...
**2.1 极速搜索取景地信息**
```bash
NODE_TLS_REJECT_UNAUTHORIZED=0 flyai fliggy-fast-search --query "[作品名称] 取景地 [主要取景城市]"
```
...
## 关于 `ask_user_question` 工具

这是 Qoder 提供的交互工具，用于在执行过程中向用户提问并收集信息。

**使用方法：**
- 每个问题可以提供 2-4 个选项
- 用户可以选择预设选项，也可以选择"其他"输入自定义内容
- 支持单选和多选（通过 `multiSelect: true` 开启）
...
问题: "你想打卡哪部作品？"
选项:
- label: "灌篮高手", description: "日本神奈川·镰仓高校前站·湘南海岸"
- label: "去有风的地方", description: "云南大理·沙溪古镇·凤阳邑"
- label: "花儿与少年", description: "挪威·卑尔根·峡湾"
- label: "其他作品", description: "我来说明具体作品名称"
```

这样可以让用户体验更流畅，同时获取准确的需求信息。
