...
| Enable OSS | `aliyun OssAdmin OpenOssService` | SKILL.md Step 2 |
...
| Clean up resources | `aliyun fc DeleteCustomDomain` / `aliyun devs delete-project` / `aliyun oss rm` | SKILL.md Cleanup |
...
| `bailian_api_key` | Bailian API Key | Auto-created via `aliyun maas create-api-key` in Step 1 |
...
> OSS service can be auto-activated via `aliyun OssAdmin OpenOssService` (already built into SKILL.md Step 2).
> Bailian workspace is auto-created via `POST /maas/workspaces` if none exists (built into SKILL.md Step 1).

## Notes

- The OSS CLI plugin does not support the `--user-agent` flag (returns `invalid flag`) — use `--ua AlibabaCloud-Agent-Skills` instead
- All other `aliyun` commands must include `--user-agent AlibabaCloud-Agent-Skills`
